...
const TARGET = path.join(__dirname, "public", "App.jsx");
let src = fs.readFileSync(TARGET, "utf8");

let changed = 0;
function replace(find, replacement, label) {
  if (!src.includes(find)) { console.warn("WARNING - ANCHOR NOT FOUND:", label); return; }
...
// 2. generateJobReportPDF — insert before waInvoiceLink
replace(
  ...
  const profit = (job.total || 0) - totMat;
  ...
  doc.setFontSize(10);
  [["Revenue", gbpFmt(job.total||0)], ["Materials", gbpFmt(totMat)], ["Profit", gbpFmt(profit)], ["Margin", margin + "%"]].forEach(([k,v],i) => {
  ...
  if (photos.length > 0) {
    ...
    const cols = 3; const imgW = (w - 28 - (cols-1)*4)/cols; const imgH = imgW*0.75;
    photos.forEach((p, i) => {
      const col = i%cols; const row = Math.floor(i/cols);
      try { doc.addImage(p, "JPEG", 14+col*(imgW+4), y+row*(imgH+4), imgW, imgH); } catch {}
    });
  ...
  doc.text((biz.name||"") + "  \u2022  Generated by JobProfit  \u2022  " + new Date().toLocaleDateString("en-GB"), w/2, fy, { align: "center" });
...
// 3. _JobReportBtn component — insert before JobDetail
replace(
...
function _JobReportBtn({ job, biz, expenses, flash }) {
  ...
  const download = async () => {
    setBusy(true);
    try { await generateJobReportPDF(job, biz, expenses); flash("PDF downloaded"); }
    catch { flash("Could not generate report. Please try again."); }
    setBusy(false);
  };
  return (
...
// 5. MonthlyOverheadsSettings component — before SettingsTab
replace(
  `/* ═══ SETTINGS ═══════════════════════════════════════ */`,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <select value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))} style={{ ...S.inp, flex: 1, appearance: "auto" }}>
              {OVERHEAD_CATEGORIES.map(c => React.createElement("option", { key: c, value: c }, c))}
            </select>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
...
// 7. Add overhead section inside SettingsTab before Reminders
replace(
  ...
  `    {/* Monthly Overheads */}
    <div style={{ marginTop: 24 }}>
      ...
      <p style={{ fontSize: 13, color: T.textMuted, margin: "0 0 14px" }}>Track fixed costs to see your true profit after overhead.</p>
...
// 8. OverviewTab — add overheads prop + pass to QuickStats
replace(
...
// 9. QuickStats — accept overheads prop + True Profit block
replace(
...
// Insert True Profit block after Performance Momentum card, before closing div of QuickStats return
replace(
  `  </div>;
}
  ...
  `  {/* True Profit After Overhead */}
    {(() => {
      ...
      if (activeOH.length === 0) return (
        <div style={{ background: T.surfaceAlt, border: "1.5px dashed " + T.border, borderRadius: T.r, padding: "12px 16px", marginTop: 10, display: "flex", alignItems: "center", gap: 10 }}>
      ...
      return (
        <div style={{ background: trueProfit >= 0 ? "linear-gradient(135deg, #1E3A8A, #1f9d55)" : "linear-gradient(135deg, #dc2626, #b91c1c)", borderRadius: T.r, padding: "18px 20px 14px", color: "#fff", boxShadow: T.heroShadow, marginTop: 10, position: "relative", overflow: "hidden" }}>
...
replace(
  ...
  "Pass overheads to OverviewTab render"
);
...
replace(
  ...
  "Pass overheads to SettingsTab render"
);
