...
function QuickStats({ todayProfit, outstanding, paidThisMonth, profitThisMonth, unpaidCount, onChase, jobs, expenses, biz, overheads }) {
  ...
  {/* True Profit After Overhead */}
    {(() => {
      ...
      return (
        <div style={{ background: trueProfit >= 0 ? 'linear-gradient(135deg, #1E3A8A, #1f9d55)' : 'linear-gradient(135deg, #dc2626, #b91c1c)', borderRadius: T.r, padding: '18px 20px 14px', color: '#fff', boxShadow: T.heroShadow, marginTop: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1.5, textTransform: 'uppercase', opacity: .8, marginBottom: 4 }}>True Profit This Month</div>
          <div style={{ fontSize: 40, fontWeight: 800, lineHeight: 1, letterSpacing: -1 }}>{GBP(trueProfit)}</div>
          <div style={{ fontSize: 12, opacity: .7, marginTop: 6, fontWeight: 500 }}>after {GBP(ohTotal)}/mo fixed costs</div>
        </div>
      );
    })()}
  </div>;
