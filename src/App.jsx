...
const defBiz = { name: "Your Business Name", phone: "07XXX XXXXXX", email: "you@email.com", address: "Your Business Address", trade: "General Builder", vatRegistered: false, vatNumber: "", hourlyRate: 45, bankDetails: "", accountName: "", sortCode: "", accountNumber: "", logoUrl: "", stripePaymentLink: "" };
...
// removed: browser-side Anthropic call (security) — no API key in the browser.
// Live AI calls go through netlify/functions/ai.js. App.jsx is dead code behind
// the debug escape-hatch (jp.navSlice3='0') so these stubs are never reached.
async function aiQuote(_text, _biz) { return null; }
async function aiReceipt(_b64raw) { return null; }
...
function Stat({ label, value, color = T.text, sub, icon, bg }) { return <div style={{ background: bg || T.surface, borderRadius: T.r, padding: "14px 16px", border: `1px solid ${T.border}`, flex: "1 1 140px", minWidth: 130, boxShadow: T.cardShadow }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>{icon && <span style={{ fontSize: 14 }}>{icon}</span>}<div style={{ fontSize: 10, fontWeight: 600, color: T.textMuted, letterSpacing: .5, textTransform: "uppercase" }}>{label}</div></div><div style={{ fontSize: 22, fontWeight: 800, color, lineHeight: 1.2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</div>{sub && <div style={{ fontSize: 12, color: T.textMuted, fontWeight: 400, marginTop: 3 }}>{sub}</div>}</div>; }
...
function SendInvoiceModal({ job, biz, profile, jobs, onUpdate, onClose, flash }) {
...
function MaterialForm({ jobs, presetJobId, showVat, onSave, onCancel }) {
  ...
  return <div>
    ...
    {!manual ? <div style={{ display: "flex", gap: 10 }}><button onClick={() => fileRef.current?.click()} disabled={busy} style={{ ...pri, flex: 1, opacity: busy ? .5 : 1 }}>{busy ? <><Spinner /> Scanning...</> : <><CamIc /> Snap Receipt</>}</button><button onClick={() => setManual(true)} style={sec}>Manual</button><input ref={fileRef} type="file" accept="image/*" onChange={handlePhoto} style={{ display: "none" }} /></div>
    : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
...
function InsightsCard({ jobs, expenses, invoices, biz }) {
  ...
  const paidThisWeek = thisWeekJobs.filter(j => (j.paymentStatus??(j.paid?'paid':'unpaid')) === "paid");
  const paidLastWeek = lastWeekJobs.filter(j => (j.paymentStatus??(j.paid?'paid':'unpaid')) === "paid");

  // KPIs
  const twRevenue = paidThisWeek.reduce((s, j) => s + (j.total??j.amount??0), 0);
  ...
  const unpaidJobs = jobs.filter(j => (j.paymentStatus??(j.paid?'paid':'unpaid')) === "unpaid" && (j.quoteStatus??'accepted') === "accepted");
  const unpaidTotal = unpaidJobs.reduce((s, j) => s + (j.total??j.amount??0), 0);

  // Best lead source
  const srcMap = {};
  paidThisWeek.forEach(j => { if (j.source) { if (!srcMap[j.source]) srcMap[j.source] = { profit: 0, count: 0 }; const mat = expenses.filter(e => e.jobId === j.id).reduce((a, e) => a + e.amount, 0); srcMap[j.source].profit += (j.total??j.amount??0) - mat; srcMap[j.source].count++; } });
  ...
  // B- Margin drop
  const lwRevenue = paidLastWeek.reduce((s, j) => s + (j.total??j.amount??0), 0);
  ...
  const allSrcMap = {};
  jobs.filter(j => (j.paymentStatus??(j.paid?'paid':'unpaid')) === "paid" && j.source).forEach(j => { if (!allSrcMap[j.source]) allSrcMap[j.source] = { revenue: 0, mat: 0, count: 0 }; allSrcMap[j.source].revenue += (j.total??j.amount??0); allSrcMap[j.source].mat += expenses.filter(e => e.jobId === j.id).reduce((a, e) => a + e.amount, 0); allSrcMap[j.source].count++; });
...
function OverviewTab({ jobs, expenses, invoices, onGo, biz, showVat }) {
  ...
  const activeJobs = jobs.filter(j => (j.jobStatus??(j.paid?'complete':'active')) === "active" || ((j.quoteStatus??'accepted') === "accepted" && (j.paymentStatus??(j.paid?'paid':'unpaid')) !== "paid")).length;
  ...
  const tmrwVal = tmrwJobs.reduce((s, j) => s + (j.total??j.amount??0), 0);
  ...
  return <div>
    ...
    {unpaid.length > 0 && <div id="chase-section" style={{ marginBottom: 22 }}><span style={S.lbl}>💰 Chase These Payments</span>{[...unpaid].sort((a, b) => { const aInv = invoices.find(i => i.jobId === a.id); const bInv = invoices.find(i => i.jobId === b.id); const aDays = aInv ? daysSince(aInv.created) : daysSince(a.date); const bDays = bInv ? daysSince(bInv.created) : daysSince(b.date); return bDays - aDays || b.total - a.total; }).slice(0, 5).map(j => { const jInv = invoices.find(i => i.jobId === j.id); const days = jInv ? daysSince(jInv.created) : daysSince(j.date); const level = chaseLevel(j, jInv); const li = chaseLevelInfo(level); const lastR = (j.reminders || []).length > 0 ? j.reminders[j.reminders.length - 1] : null; const chaseCount = (j.reminders || []).length; const nextChase = !lastR ? "Chase now" : daysSince(lastR.date) >= 3 ? "Chase again" : "Chased " + daysSince(lastR.date) + "d ago"; return <div key={j.id} style={{ ...S.card, marginBottom: 10, borderLeft: `4px solid ${li.color}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        ...
        <div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontWeight: 800, fontSize: 18, color: li.color }}>{GBP((j.total??j.amount??0))}</div></div>
      </div>
...
function CustomerDetail({ customer, onBack, onGoJob }) {
  ...
  return <div><button onClick={onBack} style={{ ...S.ghost, marginBottom: 16 }}><ChvIc d="left" /> Back</button>
    ...
    {customer.jobs.map(j => <div key={j.id} onClick={() => onGoJob(j.id)} style={{ ...S.card, marginBottom: 10 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}><div><div style={{ fontWeight: 700, fontSize: 14 }}>{(j.summary ?? '').slice(0, 80)}</div><div style={{ fontSize: 12, color: T.textMuted, marginTop: 3 }}>{j.customer || j.date}</div></div><div style={{ textAlign: "right" }}><div style={{ fontWeight: 800, fontSize: 17 }}>{GBP((j.total??j.amount??0))}</div></div></div><div style={{ display: "flex", gap: 6 }}><StatusBadge job={j} /></div></div>)}
  </div>;
}
...
function JobDetail({ job, jobs, expenses, invoices, onBack, onUpdate, onDelExp, onAddExp, onUpdateInvoice, biz, profile, showVat, onViewCustomer, flash, onAddPayment }) {
  ...
  return <div>
    ...
    {/* Quick actions */}
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}><button onClick={() => setAddingExp(true)} style={{ ...sec, width: "100%", fontSize: 12, padding: "10px 6px" }}>🧾 Material</button><button onClick={() => photoRef.current?.click()} style={{ ...sec, width: "100%", fontSize: 12, padding: "10px 6px" }}>📷 Photo</button><button onClick={() => { setEd(true); setD({ ...job }); }} style={{ ...sec, width: "100%", fontSize: 12, padding: "10px 6px" }}>✏️ Edit</button><input ref={photoRef} type="file" accept="image/*" multiple onChange={handleJobPhoto} style={{ display: "none" }} /></div>
    {/* Quote breakdown */}
    <div style={{ background: T.surface, borderRadius: T.r, padding: 20, border: `1px solid ${T.border}`, boxShadow: T.cardShadow, marginBottom: 16 }}>
...
function JobsTab({ jobs, expenses, invoices, onUpdate, onDelExp, onAddExp, onAddInvoice, onUpdateInvoice, biz, profile, showVat, onXLSX, unassigned, onGoMaterials, initialJobId, clearInitialJob, flash, onAddPayment }) {
  ...
  return <div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}><Stat label="Earned" value={GBP(paid.reduce((s, j) => s + (j.total??j.amount??0), 0))} color={T.accent} icon="✅" sub={paid.length + " paid"} /><Stat label="Outstanding" value={GBP(unpaid.reduce((s, j) => s + (j.total??j.amount??0), 0))} color={T.warn} icon="⏰" sub={unpaid.length + " unpaid"} /></div>
    ...
    : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{filtered.map(j => { const { materials: jE, profit: pr } = calcProfit(j, expenses); const jInv = invoices.find(i => i.jobId === j.id); const overdueDays = (j.paymentStatus??(j.paid?'paid':'unpaid')) === "unpaid" && jInv ? daysSince(jInv.created) : (j.paymentStatus??(j.paid?'paid':'unpaid')) === "unpaid" ? daysSince(j.date) : 0; return <div key={j.id} onClick={() => setSelId(j.id)} style={S.card}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6, gap: 8 }}><div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 15 }}>{j.customer}</div><div style={{ color: T.textMed, fontSize: 13, marginTop: 2 }}>{j.address}</div></div><div style={{ display: "flex", gap: 5, flexShrink: 0, flexWrap: "wrap", justifyContent: "flex-end" }}><StatusBadge job={j} />{j.source && <Badge text={srcIcon(j.source) + " " + j.source} bg={srcCol(j.source)} />}{(j.paymentStatus??(j.paid?'paid':'unpaid')) === "unpaid" && (j.quoteStatus??'accepted') === "accepted" && overdueDays > 0 && <Badge text={"⚠️ " + overdueDays + "d overdue"} bg={overdueDays > 14 ? T.danger : T.warn} />}</div></div><div style={{ color: T.textMed, fontSize: 13, lineHeight: 1.4, marginBottom: 10 }}>{j.summary}</div><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: `1px solid ${T.borderLight}`, paddingTop: 10 }}><span style={{ color: T.textMuted, fontSize: 12 }}>{j.customer || j.date}{j.scheduledDate && <span> · 📅 {j.scheduledDate}</span>}</span><div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>{jE > 0 && <span style={{ fontSize: 12, color: T.danger, fontWeight: 600 }}>−{GBP(jE)}</span>}<span style={{ fontWeight: 800, fontSize: 18 }}>{GBP((j.total??j.amount??0))}</span>{jE > 0 && <span style={{ fontSize: 12, color: pr >= 0 ? T.accent : T.danger, fontWeight: 800 }}>{GBP(pr)}</span>}</div></div></div>; })}</div>}
  </div>;
}
...
function ScheduleTab({ jobs, expenses, biz, onUpdate, onGo, flash }) {
  ...
  const totalVal = scheduled.reduce((s, j) => s + (j.total??j.amount??0), 0);
    ...
    : scheduled.map(j => { const { profit: pr } = calcProfit(j, expenses); const reminderToday = j.reminderSentAt && j.reminderSentAt.startsWith(today); return <div key={j.id} style={{ ...S.card, marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        <div style={{ flex: 1 }} onClick={() => onGo("Jobs", j.id)}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}><span style={{ fontSize: 12, fontWeight: 700, color: T.primary, background: T.primaryLight, padding: "2px 8px", borderRadius: 6 }}>{timeLabel(j)}</span><StatusBadge job={j} /></div>
        ...
        <div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontWeight: 800, fontSize: 24, color: T.accent, lineHeight: 1.1 }}>{GBP(pr)}</div><div style={{ fontSize: 10, color: T.textMuted, fontWeight: 500, marginTop: 2, textTransform: "uppercase" }}>profit</div><div style={{ fontSize: 13, color: T.textMed, fontWeight: 600, marginTop: 2 }}>{GBP((j.total??j.amount??0))} value</div></div>
      </div>
...
function SettingsTab({ biz, onUpd }) {
  ...
  const ch = (f, v) => { onUpd({ ...biz, [f]: v }); setLastSaved(f); setTimeout(() => setLastSaved(p => p === f ? null : p), 2000); };
  const logoRef = useRef(null);
  // Auto-build legacy bankDetails string from structured fields when any are set
  useEffect(() => {
    if (biz.accountName || biz.sortCode || biz.accountNumber) {
      ...
      if (built !== biz.bankDetails) onUpd({ ...biz, bankDetails: built });
    }
  ...
  return <div style={{ maxWidth: 500 }}>
    ...
    <div style={{ marginBottom: 14 }}><label style={{ display: "block", fontSize: 14, fontWeight: 600, color: T.textMed, marginBottom: 6 }}>Card payment link (optional)<Saved f="stripePaymentLink" /></label><div style={{ fontSize: 12, color: T.textMuted, marginBottom: 6 }}>Paste your Stripe Payment Link. Customers tap to pay by card on every invoice.</div><input type="url" value={biz.stripePaymentLink || ""} onChange={e => ch("stripePaymentLink", e.target.value)} placeholder="https://buy.stripe.com/..." style={S.inp} /></div>
...
export default function App({ cloudJobs, profile: supabaseProfile, onAddPayment } = {}) {
  ...
  useEffect(() => {
    ...
    const today = td();
    const todayJobs = jobs.filter(j => j.scheduledDate === today);
    const todayVal = todayJobs.reduce((s, j) => s + (j.total??j.amount??0), 0);
    const unpaidJobs = jobs.filter(j => (j.paymentStatus??(j.paid?'paid':'unpaid')) === "unpaid" && (j.quoteStatus??'accepted') === "accepted");
    // Daily check-in
