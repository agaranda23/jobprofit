...
describe('A. Checkbox label wording', () => {
  ...
  it('old label string is no longer the canonical wording', () => {
    // If this test fails, the forbidden string has crept back in.
  ...
  it('transparency link text is separate from the acceptance phrase', () => {
    // The transparency link is visually separated by a mid-dot — it is NOT
    // part of what the customer is agreeing to.
    expect(TRANSPARENCY_LINK_TEXT).toBe('See how your details are used');
    // The acceptance phrase itself does not reference the privacy policy.
...
// ── B. Stored acceptance string ───────────────────────────────────────────────
// The string written to jobs.meta by accept-quote.js is unchanged by PR1
// (that's a server-side constant). But the string displayed in
// RemoteAcceptedBlock (client-side, after sign) was changed.
...
describe('D. Privacy footer plan independence', () => {
  it('always shows the privacy footer for non-Pro (free) traders', () => {
    expect(shouldShowPrivacyFooter(false)).toBe(true);
  });
  ...
  it('always shows the privacy footer for Pro traders too', () => {
    ...
    expect(shouldShowPrivacyFooter(true)).toBe(true);
  });
});
