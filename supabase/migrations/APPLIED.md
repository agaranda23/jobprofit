...
| 37 | `20260706120000_referral_rewards_phase2.sql` | deferred | Adds profiles.pro_comp_until + referrals.stripe_invoice_id/rewarded_at for the JP-LU7 Phase 2 reward grant. **SELF-HEALS Phase 1 (#33) drift too** — re-declares referral_code/referred_by/referrals table+RLS with the same IF NOT EXISTS guards, so pasting this file alone is safe even if #33 was never applied. **RUN IN SUPABASE SQL EDITOR after merging feat/referral-rewards-phase2** — the reward-grant code degrades gracefully (schema_missing skip, logged) if pasted before the columns exist. |
