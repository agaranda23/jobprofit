...
| 33 | `20260623010000_add_referrals.sql` | yes (2026-07-06) | Adds profiles.referral_code + profiles.referred_by, creates referrals table with RLS (JP-LU7 Phase 1). Verified live in prod 2026-07-06 via an information_schema check (referrals table + referral_code/referred_by + referrals_unique_referee + RLS + referrals_select_own policy all present). Ledger was stale — the schema was applied all along. |
| 34 | `20260701000000_add_google_review_link_to_profiles.sql` | deferred | Adds profiles.google_review_link (nullable text) for the post-paid "Leave a Google review" CTA. **RUN IN SUPABASE SQL EDITOR after merging feat/post-paid-review-nudge** — if the column is missing, the review CTA is silently replaced by the settings nudge (graceful degradation). |
| 35 | `20260701100000_realtime_jobs_publication.sql` | deferred | Adds jobs table to supabase_realtime publication + confirms RLS SELECT policy exists. **This file is documentation only — SQL must be run manually in Supabase Dashboard → SQL Editor.** App is correct without it (Fix A + Fix B backstop guarantee correctness); realtime delivery makes the UX faster. See migration file for exact SQL and caveats. |
| 36 | `20260706000000_add_books_share_token.sql` | deferred | Adds profiles.books_share_token (uuid) + books_share_created_at for the read-only accountant "books link" (feat/accountant-books-link). No RLS change — existing profiles_select_own/update_own cover it; no anon policy added. **RUN IN SUPABASE SQL EDITOR after merging** — the Settings row degrades to "can't generate a link yet" until the column exists. |
| 37 | `20260706120000_referral_rewards_phase2.sql` | yes (2026-07-06) | Adds profiles.pro_comp_until + referrals.stripe_invoice_id/rewarded_at for the JP-LU7 Phase 2 reward grant. **SELF-HEALS Phase 1 (#33) drift too** — re-declares referral_code/referred_by/referrals table+RLS with the same IF NOT EXISTS guards, so pasting this file alone is safe even if #33 was never applied. Applied to prod 2026-07-06 (adds profiles.pro_comp_until + referrals.stripe_invoice_id/rewarded_at for the JP-LU7 Phase 2 reward grant). |
| 38 | `20260709000000_add_receipt_items_cascade_delete.sql` | deferred | Belt-and-braces for the receipt-delete orphan bug (fix/receipt-delete-cascade): deletes any already-orphaned receipt_items rows, then ensures receipt_items.receipt_id has an ON DELETE CASCADE FK (drops+recreates whatever FK exists if it isn't already CASCADE). App-code half (explicit receipt_items delete in deleteReceiptFromCloud) already ships correctness independent of this migration — **RUN IN SUPABASE SQL EDITOR after merging**, not launch-blocking. |
| 39 | `20260724000000_add_campaigns.sql` | deferred | Influencer/affiliate campaign codes (JP-LU9): creates campaigns table (typed codes e.g. MITCH60, not tied to a profiles row), relaxes referrals.referrer_id to nullable, adds referrals.campaign_id + bounty_* accrual/clawback columns. **RUN IN SUPABASE SQL EDITOR before campaign codes go live** — record-referral.js / referralReward.js / campaignBounty.js all degrade gracefully (42703/42P01 → no-op) until this is applied, so merging the code first is safe. See feat/influencer-campaign-codes PR description. |
